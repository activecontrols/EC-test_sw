...
Vector4 ASTRAv2_Controller(Vector3 PosTarget, Vector16 X, constantsASTRA_t constantsASTRA, float dT) {
  ...
  // Velocity Command
  Vector3 K_P = (Vector3() << 0.6, 0.6, 0.65).finished();
  ...
  Vector3 K_I = (Vector3() << 0.1, 0.1, 1).finished();
  float Leak = 0.2;
  Vector3 Clamp = (Vector3() << 5, 5, 5).finished();

  // Normalize errors (0 to 1 scale)
  Vector3 MaxAttError = (Vector3() << 0.1, 0.1, 0.3).finished();
  Vector3 MaxVelError = (Vector3() << 0.4, 0.4, 0.6).finished();
  ...
  K_P = (Vector3() << 2.2, 2.2, 5).finished();
